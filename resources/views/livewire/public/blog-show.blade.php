    ...
    @if ($related->isNotEmpty())
        <div class="border-t border-ink-200 bg-white py-14">
            <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                <h2 class="font-serif text-xl font-medium text-ink-900">{{ __('Related posts') }}</h2>
                <ul class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    @foreach ($related as $rel)
                        <li>
                            <a href="{{ route('blog.show', $rel->slug) }}" class="block rounded-md border border-ink-200 bg-white p-4 hover:border-brand-300">
                                <p class="font-medium text-ink-900">{{ $rel->getTranslation('title', app()->getLocale()) }}</p>
                            </a>
                        </li>
                    @endforeach
                </ul>
