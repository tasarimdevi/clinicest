...
/**
 * A demo clinic, doctor, and reviews so the homepage and directories
 * render meaningfully in local dev. Not representative of production
 * content — see docs/10-roadmap.md Phase 1 for real onboarding volume
 * targets. Unlike RealContentSeeder (countries/treatments/FAQs/posts,
 * which this depends on and which IS safe for production), everything
 * below is a fabricated business/person and must stay dev/staging-only
 * per docs/10-roadmap.md §3 ("never fabricate trust signals").
 */
class DemoDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $gb = Country::where('slug', 'uk')->firstOrFail();
        $de = Country::where('slug', 'germany')->firstOrFail();
        $istanbul = City::where('slug', 'istanbul')->firstOrFail();
        $treatments = Treatment::whereIn('slug', [
            'dental-implants', 'all-on-4', 'hollywood-smile', 'veneers',
        ])->get()->keyBy('slug');
        ...
        $ie = Country::where('slug', 'ireland')->firstOrFail();
        $clinicReviews = [
