...
class ChatWidget extends Component
{
    ...
    public function send(ChatTools $tools): void
    {
        ...
        $response = $groq->chat($groqMessages, ['tools' => $tools->definitions()]);
        ...
        ChatMessage::create([
            ...
            'original_draft' => $verified['flagged'] ? $draft : null,
            'tool_name' => $toolName,
        ...
        $session->increment('token_count', $tokensUsed);
        $settings->recordTokensUsed($tokensUsed);

        $this->messages[] = ['role' => 'assistant', 'content' => $verified['content']];
    }
